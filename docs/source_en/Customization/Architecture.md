...
For continuous weights based on the original Agent roles, use `--loss_scale role --role_loss_config role_loss_config.json`. The role strategy reads `loss_scale` from `system`, `user`, `assistant`, `tool_call`, and `tool_response` messages. The config can additionally set relative weights for assistant think tags, think content, and answers. It follows the existing `LossScale` convention of splitting text and assigning weights before tokenization; empty think blocks and multimodal input tokens retain the framework's default masks.
